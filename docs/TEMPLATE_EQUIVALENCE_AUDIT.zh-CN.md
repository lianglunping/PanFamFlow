...
当前 PanFamFlow 已为 PDF/MD 要求建立完整的代码与产物合同，但**尚不能把工程实现写成真实研究交付完成**。权威 58 项能力矩阵回答“执行路径和规范输出是否存在”，逐图矩阵回答“Fig01–Fig34 是否有源表及 PDF/PNG 合同”；两者都不能替代 clean run、原生工具链、公共数据 provenance 和生物学验收。

在 B10 与 14 门验收完成前，当前 revision **仍不与 PDF 模板或 MD 完整交付清单等价**；它只达到“实现合同完整、最终执行证据待验收”的阶段。
...
因此，更准确的表达是：**58 个主题均有可执行/条件可执行路径；Fig01–Fig34 共用 33 条审计记录（Fig21–22 合并），其中 28 条走默认完整交付路径、5 条走默认关闭的可选路径。当前仍在等待 B10 clean run、昆鹏原生工具链、无工作复跑、局部恢复和 14 门最终验收。**
...
- 构建覆盖默认和全部可选路径的 `toy_complete`，从 clean directory 生成 Fig01–Fig34、同源 TSV/XLSX、PDF/600 dpi PNG、版本、session、provenance 和 manifests；
- 用相同配置复跑并证明 `Nothing to be done`；删除一个声明的子产物后，只恢复该依赖闭包；
- 在昆鹏用冻结 linux-64 lock、原生 JCVI/OrthoFinder/KaKs 等工具链及固定 DESeq2 容器完成调度证据；
- 核验组织、非生物胁迫和生物胁迫公共水稻数据的 accession、样本/run 映射、参考版本、URL、校验和与文件大小；
- 在同一最终 revision 上通过 14 项验收门后，才允许推送、部署和更新公共站点完成状态。

## 当前验收口径

当前可描述为“PDF/MD 的分析主题、执行路径和产物合同已经无目录遗漏”；尚不可描述为“所有分析均已在真实水稻数据上验证”或“最终交付已经完成”。最终结论以 B10 clean run、昆鹏原生证据和 14 门验收为准。
