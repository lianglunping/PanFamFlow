...
| 状态 | 数量 | 含义 |
|---|---:|---|
| `IMPLEMENTED` | 53 | 存在直接可达的规则/脚本、规范源表与结果合同；输入不足时必须产生明确 QC 状态，而不是伪造结果。 |
| `CONDITIONALLY_AVAILABLE` | 5 | 执行路径已经实现，但默认关闭或需要额外、可审计的结构域对齐、全基因组共线性或 raw-count 实验设计输入。 |

5 项条件实现为：4.4 核心结构域 Logo、8.6 全基因组共线性，以及 11.3–11.5 raw-count 差异表达/胁迫整合。6.1 已拆分为可直接到达且命名边界固定的 OrthoFinder 物种系统树与目标家族 OGG 有无聚类；后者明确不是系统发育树。`CONDITIONALLY_AVAILABLE` 表示“代码路径存在但运行门禁未必满足”，不是“尚未开发”。
