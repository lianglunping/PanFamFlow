...
| 状态 | 数量 | 含义 |
|---|---:|---|
| `IMPLEMENTED` | 21 | 存在可执行路径、明确的规范输出，并能直接回答该条目的核心问题。 |
| `CONDITIONALLY_AVAILABLE` | 29 | 已有可执行路径或基础数据，但仍依赖亚家族/群体元数据、有效配对、足够物种重复或额外领域前提；不能无条件表述为自动完成。 |
| `EXTERNAL_IMPORT` | 2 | PanFamFlow 只接收外部结果用于整合或展示，不负责生成其统计证据。 |
| `NOT_SUPPORTED` | 6 | 当前没有足以支撑该条目结论的执行路径或规范输出。 |
